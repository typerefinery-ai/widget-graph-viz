...
## Visualization Loader (Overview)

- The new loader (`src/js/_widget.loader.js`) is responsible for selecting and mounting visualization modules. It watches `[data-widget-root]` by default, with `[component="graphviz"]` retained as a fallback for existing markup.
- Visualization modules register through `WidgetLoader.registerVisualization` (see `src/js/modules/compose/`, `src/js/modules/overview/`, and `src/js/modules/example/`).
- Module resolution order: `data-visualization` attribute → `?viz=<id>` query parameter → loader-configured default → first registered module.
- `widget.js` now primarily preserves legacy compat helpers (`window.Widgets.Widget.*`) so existing panel code can continue to call `raiseEventDataRequest`, `loadData`, and utility helpers while the loader orchestrates modules.
